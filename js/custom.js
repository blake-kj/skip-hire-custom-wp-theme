...
function headerProductKeyUp(obj) {
  ...
  if (val) {
      searchResults.forEach(function (location) {
          ...
          // /product/4-cubmic-metre-skip/?attribute_depo=Pinjarra&attribute_distance=0-25km
          // location['Depo'] 
          // distance
...
function headerShortcodeProducts(obj) {
  ...
  jQuery('.depo-price').each(function () {
    if ((jQuery(this).data('depo') === depo) && (jQuery(this).data('distance') === distance)) {
      ...
      jQuery(this).parent().parent().attr('href', `/?add-to-cart=${productid}&variation_id=${variationid}&attribute_depo=${depo}&attribute_distance=${distance}`);
...
function exampleContent() {
  ...
  searchResults = locations.slice(0, 4);
